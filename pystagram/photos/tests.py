...
class PhotoTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client = Client()
        self.image_file_path = os.path.join(settings.MEDIA_ROOT, 'lynn01.jpg')
    ...
    def test_save_photo_by_model(self):
        ...
        # user가 없어서 오류가 발생하는 테스트.
        new_photo2 = Photo()
        new_photo2.image = self.image_file_path
        new_photo2.description = ''

        # user에 제대로된 값을 할당하지 않았으므로 ValueError 발생.
    ...
    def test_save_photo_by_model_with_form(self):
        ...
        # 폼에 첫 번째 인자로 POST로 넘어온 폼 값, 
        # 두 번째 인자로 POST로 업로드 된 파일 전다f
    ...
    def test_view_get_create_photo(self):
        ...
        url = '/photos/create/'
        # 로그인 안하고 접속하기
        # login_required가 redirect하는 걸 따라가야하므로 follow=True
        # 참고 response attributes: https://goo.gl/elVe2c
        res = self.client.get(url, follow=True)
        # redirect되어 도착한 곳은 200. 응답
        self.assertEqual(res.status_code, 200)
        # redirect 되어 도착한 곳의 뷰 함수 이름은 login이어야 한다.
    ...
    def test_view_post_create_photo(self):
        ...
        # from 컨텍스트 변수의 image 폼 필드에 오류 내용이 있는지 확인
